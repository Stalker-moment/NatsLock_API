...
router.post("/locker/request/rent", (req, res) => {
  ...
  const lockerfindsystem = systemData.Locker.find(
    (locker) => locker[locker_Method] === locker_value
  );
  ...
  if (locker.state === false) {
    return res
      .status(400)
      .json({ code: 400, message: "Locker is not available" });
  }
  ...
  if (rent) {
    if (rent.Rent.length >= 2) {
      return res.status(400).json({
        code: 400,
        message: "Maximum rent locker is 2",
      });
    }
