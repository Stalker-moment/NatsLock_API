...
<body>
  ...
  transporter.sendMail(mailOptions, function (error, info) {
    if (error) {
      ...
      console.log(chalk.magenta(TimeAndDate()), "Email sent: " + info.response + " to " + email + " about locker action Unrent");
...
router.post("/locker/verify/unrent", async(req, res) => {
  ...
  fs.writeFileSync(lockerdb, JSON.stringify(lockerData, null, 2));
   ...
   sendEmail(user.Name, user.Username, getNumLocker, Time, Date, lockerfindsystem.Temperature, lockerfindsystem.Humidity, locker.OnGoing.duration);
 
   console.log(chalk.magenta(TimeAndDate()), chalk.green(`Locker Action Unrent: ${locker.alias} by ${user.username}`))
