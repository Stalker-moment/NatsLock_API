...
<body>
  ...
  transporter.sendMail(mailOptions, function (error, info) {
    if (error) {
      ...
      console.log(chalk.magenta(TimeAndDate()), "Email sent: " + info.response + " to " + email + " about locker action Open");
...
router.post("/locker/verify/open", async(req, res) => {
  ...
  locker.expiredOpen = currentDate.getTime() + 10000;//10sec
  ...
  const findRent = rentData.find((rent) => rent.Username === user.Username);
  if (findRent) {
    findRent.History.push(added_history_rent);
  ...
  fs.writeFileSync(lockerdb, JSON.stringify(lockerData, null, 2));
   ...
   console.log(chalk.magenta(TimeAndDate()), chalk.green(`Locker Action Open: ${locker.alias} by ${user.username}`))
